function [Ksc,D,d] = makeScalpCond(ptot,ttot,estimates,patterns,elecs)
    ...
    scalp1 = zeros(size(estimates,1),3);
    skull1 = zeros(size(estimates,1),3);
    scalp2 = zeros(size(estimates,1),3);
    ...
    for i = 1:size(estimates,1)
        ...
        %scalp1(i,1:3) = centroids2(PQ,1:3);
        scalp1(i,1:3) = elec_centers(patterns(i,1),:);%centroids2(PQ,1:3);
        

        % closest csf centroids3 to skull centroid
        PQ = dsearchn(centroids3,scalp1(i,:));
        skull1(i,1:3) = centroids3(PQ,1:3);

        % closest skull to csf
        PQ = dsearchn(centroids2,skull1(i,:));
        scalp2(i,1:3) = centroids2(PQ,1:3);
    ...
    F1 = scatteredInterpolant([scalp1; scalp2],[estimates1; estimates1],'natural','linear');
    VQ = F1(centroids2);
    D1= VQ;
    D1(isnan(D1))=1;
    D1(D1<0.05)= 0.0001;
    ...
    np = size(p,1);
    L=length(elecs);
    Ksc = sparse([femeg_stiffness(p,t,D),zeros(np,L);zeros(L,np+L)]);
    
    d = zeros(size(p,1),1);
    t_p = unique(t(t(:,5)==1,1:4));
    VQ2 = F1(p(t_p,1:3));
    d(t_p) = VQ2;
